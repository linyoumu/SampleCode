...
{
    if (self.delegateForContainer && [self.delegateForContainer respondsToSelector:@selector(facialViewDidScrollToPage:)]) {
        [self.delegateForContainer facialViewDidScrollToPage:page];
    }
}
...
{
    return [_facialView.faces indexOfObject:string];
}

- (NSString *)faceStringAtIndex:(NSUInteger)index
{
    if (index < _facialView.faces.count) {
        return _facialView.faces[index];
    }
    return nil;
}
